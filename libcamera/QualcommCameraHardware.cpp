...
board_property boardProperties[] = {
        {TARGET_MSM7227, 0x00000fff, false, false, false},
...
void QualcommCameraHardware::initDefaultParameters()
{
 ...
 if( mCfgControl.mm_camera_is_supported(CAMERA_PARM_FPS_MODE)){
	...
	/* add to init focus distance */
	{
		String8 str;
		float x = 1.0; //default values
		char buffer[32];
		sprintf(buffer, "%f", x);
		str.setTo(buffer);
		sprintf(buffer, ",%f", x);
		...
		sprintf(buffer, ",%f", x);
		str.append(buffer);
		LOGV("%s: setting KEY_FOCUS_DISTANCES as %s", __FUNCTION__, str.string());
...
status_t QualcommCameraHardware::startPreviewInternal()
{
   ...
   if(mZslEnable){
    ...
    //If startPreviewInternal() has been finished, set mPreviewIsEnalbedFlag to true;
    mPreviewIsEnalbedLock.lock();
    mPreviewIsEnalbedFlag = true;
    mPreviewIsEnalbedLock.unlock();
...
void QualcommCameraHardware::stopPreviewInternal()
{
    ...
    //stopPreviewInternal() has been finished, set the mPreviewIsEnalbedFlag flag.
    mPreviewIsEnalbedLock.lock();
    mPreviewIsEnalbedFlag = false;
    mPreviewIsEnalbedLock.unlock();
...
void QualcommCameraHardware::receive_camframe_error_esd(void) {
    ...
    //If preview is disable, ignore the esd recovery
    mPreviewIsEnalbedLock.lock();
    if(!mPreviewIsEnalbedFlag){
        LOGI("receive_camframe_error_esd - preview is disable, just return");
        mPreviewIsEnalbedLock.unlock();
        return;
    }
    mPreviewIsEnalbedLock.unlock();
    ...
    //check again, if preview is disable, ignore the esd recovery
